...
std::shared_ptr<location::service::Runtime> location::service::Runtime::create(std::uint32_t pool_size)
{
    return std::shared_ptr<location::service::Runtime>(new location::service::Runtime(pool_size));
}

location::service::Runtime::Runtime(std::uint32_t pool_size)
...
location::service::Runtime::~Runtime()
{
    try
    {
        stop();
...
std::function<void(std::function<void()>)> location::service::Runtime::to_dispatcher_functional()
{
...
boost::asio::io_service& location::service::Runtime::service()
{
    return service_;
}

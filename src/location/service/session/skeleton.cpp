...
location::service::session::Skeleton::Skeleton(const location::service::session::Skeleton::Configuration& config)
...
location::service::session::Skeleton::~Skeleton() noexcept
{
...
void location::service::session::Skeleton::on_start_position_updates(const core::dbus::Message::Ptr& msg)
{
    ...
    try
    {
        configuration.local.impl->updates().position_status = location::service::session::Interface::Updates::Status::enabled;
...
void location::service::session::Skeleton::on_stop_position_updates(const core::dbus::Message::Ptr& msg)
{
    ...
    try
    {
        configuration.local.impl->updates().position_status = location::service::session::Interface::Updates::Status::disabled;
        reply = dbus::Message::make_method_return(msg);
    } catch(const std::runtime_error& e)
    {
        // We only provide a generic error message to avoid leaking
...
void location::service::session::Skeleton::on_start_heading_updates(const core::dbus::Message::Ptr& msg)
{
...
void location::service::session::Skeleton::on_stop_heading_updates(const core::dbus::Message::Ptr& msg)
{
    VLOG(10) << "MethodHandler for Interface::StopHeadingUpdates";
    auto reply = the_empty_reply();
    try
    {
        configuration.local.impl->updates().heading_status = location::service::session::Interface::Updates::Status::disabled;
        reply = dbus::Message::make_method_return(msg);
    } catch(const std::runtime_error& e)
    {
        // We only provide a generic error message to avoid leaking
...
void location::service::session::Skeleton::on_start_velocity_updates(const core::dbus::Message::Ptr& msg)
{
    VLOG(10) << "MethodHandler for Interface::StartVelocityUpdates";
    auto reply = the_empty_reply();
    try
    {
        configuration.local.impl->updates().velocity_status = location::service::session::Interface::Updates::Status::enabled;
        reply = dbus::Message::make_method_return(msg);
    } catch(const std::runtime_error& e)
    {
        // We only provide a generic error message to avoid leaking
...
void location::service::session::Skeleton::on_stop_velocity_updates(const core::dbus::Message::Ptr& msg)
{
    VLOG(10) << "MethodHandler for Interface::StopVelocityUpdates";
    auto reply = the_empty_reply();
    try
    {
        configuration.local.impl->updates().velocity_status = location::service::session::Interface::Updates::Status::disabled;
        reply = dbus::Message::make_method_return(msg);
    } catch(const std::runtime_error& e)
    {
        // We only provide a generic error message to avoid leaking
...
void location::service::session::Skeleton::on_position_changed(const location::Update<location::Position>& position)
{
    VLOG(10) << __PRETTY_FUNCTION__;
    try
    {
        configuration.remote.object->invoke_method_asynchronously_with_callback<location::service::session::Interface::UpdatePosition, void>([](const core::dbus::Result<void>& result)
        {
            if (result.is_error())
            {
...
void location::service::session::Skeleton::on_heading_changed(const location::Update<location::Heading>& heading)
{
    VLOG(10) << __PRETTY_FUNCTION__;
    try
    {
        configuration.remote.object->invoke_method_asynchronously_with_callback<location::service::session::Interface::UpdateHeading, void>([](const core::dbus::Result<void>& result)
        {
            if (result.is_error())
            {
...
void location::service::session::Skeleton::on_velocity_changed(const location::Update<location::Velocity>& velocity)
{
    VLOG(10) << __PRETTY_FUNCTION__;
    try
    {
        configuration.remote.object->invoke_method_asynchronously_with_callback<location::service::session::Interface::UpdateVelocity, void>([](const core::dbus::Result<void>& result)
        {
            if (result.is_error())
            {
...
const dbus::types::ObjectPath& location::service::session::Skeleton::path() const
{

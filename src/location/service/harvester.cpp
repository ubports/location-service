...
void location::service::Harvester::report_position_update(const location::Update<location::Position>& update)
{
    ...
    std::vector<com::ubuntu::location::connectivity::WirelessNetwork::Ptr> visible_wifis;
    config.connectivity_manager->enumerate_visible_wireless_networks([&visible_wifis](com::ubuntu::location::connectivity::WirelessNetwork::Ptr wifi)
    {
        VLOG(10) << "Got a visible wifi: " << *wifi << std::endl;
        visible_wifis.push_back(wifi);
    });

    std::vector<com::ubuntu::location::connectivity::RadioCell::Ptr> connected_cells;
    config.connectivity_manager->enumerate_connected_radio_cells([&connected_cells](com::ubuntu::location::connectivity::RadioCell::Ptr cell)
    {

...
cul::BagOfProviders::BagOfProviders(const std::set<cul::Provider::Ptr>& providers)
      ...
      providers{providers}
{
    ...
    {
        connections.push_back(provider->updates().position.connect([this](const cul::Update<cul::Position>& update)
        {
            mutable_updates().position(update);
        }));

        connections.push_back(provider->updates().heading.connect([this](const cul::Update<cul::Heading>& update)
        {
            mutable_updates().heading(update);
        }));

        connections.push_back(provider->updates().velocity.connect([this](const cul::Update<cul::Velocity>& update)
        {
...
// We always match :)
bool cul::BagOfProviders::matches_criteria(const cul::Criteria&)
{
    return true;
}

// We forward all events to the other providers.
void cul::BagOfProviders::on_wifi_and_cell_reporting_state_changed(cul::WifiAndCellIdReportingState state)
{
    for (auto provider : providers)
        provider->on_wifi_and_cell_reporting_state_changed(state);
}

void cul::BagOfProviders::on_reference_location_updated(const cul::Update<cul::Position>& position)
{
    for (auto provider : providers)
        provider->on_reference_location_updated(position);
}

void cul::BagOfProviders::on_reference_velocity_updated(const cul::Update<cul::Velocity>& velocity)
{
    for (auto provider : providers)
        provider->on_reference_velocity_updated(velocity);
}

void cul::BagOfProviders::on_reference_heading_updated(const cul::Update<cul::Heading>& heading)
{
    for (auto provider : providers)
        provider->on_reference_heading_updated(heading);
}

void cul::BagOfProviders::start_position_updates()
{
    for (auto provider : providers)
        provider->state_controller()->start_position_updates();
}

void cul::BagOfProviders::stop_position_updates()
{
    for (auto provider : providers)
        provider->state_controller()->stop_position_updates();
}

void cul::BagOfProviders::start_heading_updates()
{
    for (auto provider : providers)
        provider->state_controller()->start_heading_updates();
}

void cul::BagOfProviders::stop_heading_updates()
{
    for (auto provider : providers)
        provider->state_controller()->stop_heading_updates();
}

void cul::BagOfProviders::start_velocity_updates()
{
    for (auto provider : providers)
        provider->state_controller()->start_velocity_updates();
}

void cul::BagOfProviders::stop_velocity_updates()
{
    for (auto provider : providers)
